...
if (empty($_SESSION['admin_hub_auth']) && ($_POST['password'] ?? '') !== 'Rain0nin') {
    ...
    button{background:#2dd4bf;color:#0f172a;border:none;padding:.75rem 1.5rem;border-radius:4px;cursor:pointer;font-weight:600}
    .error{color:#ef4444;margin-bottom:1rem}</style>
    </head><body><form method="post"><h2>Lead Approvals</h2>
    <?php if(isset($error)):?><p class="error"><?=$error?></p><?php endif;?>
    <input type="password" name="password" placeholder="Password" autofocus>
    <button type="submit">Login</button></form><!-- CRM Analytics -->
