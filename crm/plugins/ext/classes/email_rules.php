...
class email_rules
{
    ...
    function prepare_subitems_table($entities_id, $fields_row, $report_id = 0)
    {        
        global $app_entities_cache, $app_fields_cache, $sql_query_having;

        if (!isset($app_entities_cache[$entities_id]) or $app_entities_cache[$entities_id]['parent_id'] != $this->entity_id)
            return false;

        $fields_list = [];
        foreach (explode(',', $fields_row) as $field_id)
        {
        ...
        $listing_sql_query = $report_id>0 ? reports::add_filters_query($report_id) : '';
        ...
        $order_info_query = db_query("select listing_order_fields from app_reports where id='" . $report_id . "'");
        $order_info = db_fetch_array($order_info_query);
        if($order_info and strlen($order_info['listing_order_fields']) > 0)
        {
            $info = reports::add_order_query($order_info['listing_order_fields'], $entities_id);
        ...
        $items_query = db_query("select e.* " . fieldtype_formula::prepare_query_select($entities_id, '') . " from app_entity_" . $entities_id . " e {$listing_sql_query_join} where e.parent_item_id='" . $this->item_info['id'] . "' {$listing_sql_query}", false);
        while ($items = db_fetch_array($items_query))
        {
            $html .= '<tr>';
    ...
    function prepare_subitems($text)
    {
        ...
        if (preg_match_all('/{#(\w+):([^}]*)}/', $text, $matches))
        {
            ...
            foreach ($matches[1] as $matches_key => $matches_text)
            {
                ...
                if (preg_match("/<(.+)>/", $pattern, $fmatches))
                {                                        
                    $report_id = (int)str_replace($fmatches[0], '', $pattern);
                                                            
                    if ($html = $this->prepare_subitems_table($entities_id, $fmatches[1], $report_id))
                    {
