...
class kanban
{
    ...
    {
        global $app_path, $app_entities_cache, $app_user;
        ...
        {            
            ...
            {
                $update_icon = '<a href="#"  onClick="open_dialog(\'' . url_for('items/form', 'id=' . $items['id'] . '&path=' . $app_path . '&redirect_to=' . $redirect_to . $reports['id']) . '\')"><i class="fa fa-edit"></i></a>';
                
                if(users::has_access('update_creator', $access_rules->get_access_schema()) and $items['created_by'] != $app_user['id'])
                {
                    $update_icon = '';
                }
                        
                $action_buttons .= $update_icon;
            }
