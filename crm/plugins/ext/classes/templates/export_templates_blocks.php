...
class export_templates_blocks
{
    ...
    function prepare_template_php_value($blocks, $item)
    {
        ...
        $fontStyle = [
                ...
                'bold' =>$this->isBold($settings),
                'italic' =>$this->isItalic($settings),
                'underline' =>$this->isUnderline($settings),            
            ];
        ...
        $output = array_map(function($v){ return strip_tags(trim($v),'<b><i><u>'); },$output);
        ...
        {
            $this->addHtmlText($text, trim($value), $fontStyle); 
    ...
    function addHtmlText($text, $value, $fontStyle)
    {
        $output = preg_split("'(<b[^>]*>.*?</b>)|(<u[^>]*>.*?</u>)|(<i[^>]*>.*?</i>)'siu",$value, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        //echo $value;
        //print_rr($output);                
        
        foreach($output as $v)
        {
            if(preg_match('/color:([^"|^;]+)"/i', $v, $color))
            {                                
                $fontStyle['color'] = $color[1];
            }
            ...
            $text->addText(strip_tags($v), $fontStyle);
