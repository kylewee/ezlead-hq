...
if($parent_entity_item_id 
        and $role = user_roles::getRolseAccessSchema($parent_entity_id, $parent_entity_item_id, 'roles_entities_access')
        and isset($role[$reports['entities_id']]))
{
    $current_access_schema  = $access_schema = $role[$reports['entities_id']];                
}

$is_kanban_sotrtable = false;

//print_rr($access_schema);

if (users::has_access('update', $access_schema) and !users::has_access('update_creator', $access_schema) and $app_fields_cache[$reports['entities_id']][$reports['group_by_field']]['type'] != 'fieldtype_autostatus')
{
