...
switch($app_module_action)
{
    case 'export':
        ...
        while($item = db_fetch_array($items_query))
        {
            ...
            $log_fields_query = db_query("select f.*, lf.value as fields_value, lf.previous_value, fr.sort_order as form_rows_sort_order,right(f.forms_rows_position,1) as forms_rows_pos from app_ext_track_changes_log_fields lf, app_fields f  left join app_forms_rows fr on fr.id=LEFT(f.forms_rows_position,length(f.forms_rows_position)-2) left join app_forms_tabs t on f.forms_tabs_id=t.id   where log_id='" . $item['id'] . "'  and f.id=lf.fields_id order by t.sort_order, t.name, fr.sort_order, forms_rows_pos, f.sort_order, f.name");
            ...
            {
                $html_fields = substr(str_replace("\n\n","\n",$html_fields),0,-1);
            }
